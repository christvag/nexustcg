...
def main():
    ...
    logger.info("=" * 60)
    logger.info("POKEMON COMPLETE DATABASE SCRAPER - STARTING")
    logger.info("=" * 60)
    
    # Initialize database
    db = SQLiteManager("pokemon_complete.db")
    if not db.connect():
    ...
    # Scrape cards
    logger.info("Starting Pokemon scraping...")
    ...
    logger.info("=" * 60)
    logger.info("POKEMON SCRAPING COMPLETED")
    logger.info("=" * 60)
    ...
    # Show database stats
    db.cursor.execute("SELECT set_name, COUNT(*) as count FROM trading_cards WHERE card_game = 'Pokemon TCG' GROUP BY set_name ORDER BY count DESC LIMIT 10")
    logger.info("Top 10 sets by card count:")
    for row in db.cursor.fetchall():
        logger.info(f"  {row['set_name']}: {row['count']} cards")
    
    # Show rarity distribution
    db.cursor.execute("SELECT rarity, COUNT(*) as count FROM trading_cards WHERE card_game = 'Pokemon TCG' GROUP BY rarity ORDER BY count DESC LIMIT 10")
    logger.info("\nTop rarities:")
    for row in db.cursor.fetchall():
        logger.info(f"  {row['rarity']}: {row['count']} cards")
    
    # Show cards with years
    db.cursor.execute("SELECT set_name FROM trading_cards WHERE card_game = 'Pokemon TCG' AND set_name LIKE '%(%)%' LIMIT 5")
    logger.info("\nSample sets with years:")
    for row in db.cursor.fetchall():
