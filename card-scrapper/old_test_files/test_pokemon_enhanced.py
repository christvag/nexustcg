...
from scrapers.pokemon_scraper_enhanced import PokemonScraperEnhanced
import logging
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("POKEMON ENHANCED SCRAPER - STARTING")
    logger.info("=" * 60)
    
    # Initialize database
    db = SQLiteManager("pokemon_enhanced.db")
    if not db.connect():
    ...
    # Initialize scraper
    scraper = PokemonScraperEnhanced()
    ...
    # Scrape cards
    logger.info("Starting Pokemon enhanced scraping...")
    ...
    logger.info("=" * 60)
    logger.info("POKEMON ENHANCED SCRAPING COMPLETED")
    logger.info("=" * 60)
    ...
    # Show database stats
    db.cursor.execute("SELECT set_name, COUNT(*) as count FROM trading_cards WHERE card_game = 'Pokemon TCG' GROUP BY set_name ORDER BY count DESC LIMIT 5")
    logger.info("Top 5 sets by card count:")
    ...
    # Show sample cards with images
    db.cursor.execute("SELECT card_name, set_name, card_number, card_image_url FROM trading_cards WHERE card_game = 'Pokemon TCG' AND card_image_url IS NOT NULL LIMIT 3")
    logger.info("\nSample cards with images:")
    for row in db.cursor.fetchall():
        logger.info(f"  {row['card_name']} | {row['set_name']} | #{row['card_number']}")
        logger.info(f"    Image: {row['card_image_url']}")
