...
class YuGiOhScraperImproved(BaseScraper):
    def __init__(self):
    ...
    def scrape_complete_api(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Use requests directly with proper timeout and streaming for large responses
            ...
            logger.info("Processing large API response...")
    ...
    def format_api_card(self, card: Dict, card_set: Dict = None) -> Dict[str, Any]:
        try:
            ...
            if card_set:
                ...
                race = card.get('race', '')
                rarity = f"{card_type} - {race}" if card_type and race else card_type or race or "Unknown"
            ...
            # Additional card info for better identification
            card_desc = card.get('desc', '')[:100] + '...' if card.get('desc') else None
    ...
    def get_card_details(self, card: Dict) -> str:
        """Get additional card details for logging/debugging"""
        details = []
        if card.get('type'):
            details.append(f"Type: {card['type']}")
        if card.get('atk'):
            details.append(f"ATK: {card['atk']}")
        if card.get('def'):
            details.append(f"DEF: {card['def']}")
        if card.get('level'):
