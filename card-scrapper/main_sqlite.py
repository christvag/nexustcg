...
from scrapers.yugioh_scraper_simple import YuGiOhScraperSimple
...
class CardScraperOrchestrator:
    def __init__(self, db_path: str = "trading_cards.db"):
        ...
        self.scrapers = {
            'pokemon': PokemonScraperFixed,
            'yugioh': YuGiOhScraperSimple,
            'mtg': MTGScraperComplete,
    ...
    def run(self, games: List[str] = None):
        ...
        logger.info("=" * 60)
        logger.info("TRADING CARD SCRAPER - STARTING (SQLite)")
        logger.info("=" * 60)
...
def main():
    parser = argparse.ArgumentParser(
        description='Scrape trading card game information and store in SQLite'
    )
    parser.add_argument(
        '--games',
        nargs='+',
    ...
    try:
        orchestrator = CardScraperOrchestrator(db_path=args.db)
